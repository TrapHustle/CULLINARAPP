...
interface StatePayload {
  ...
  /**
   * Déroulé du vote, par catégorie de table : "BY_CANDIDATE" (un candidat à la
   * fois) ou "BY_JUROR" (tous les candidats ouverts en même temps).
   *
   * Les deux sont transmis parce que la règle dépend de la table servie, et
   * qu'une même tablette peut en couvrir des deux catégories : elle seule sait
   * laquelle elle sert à cet instant.
   */
  voteModePublic: string;
  voteModeSpecial: string;
  /**
   * Candidats dont les votes ont été ouverts au moins une fois, tant que le
   * scrutin est ouvert. C'est ce que peut noter une table en « juré après
   * juré » — on ne note pas un plat qui n'a pas été servi.
   */
  openedCandidateIds: string[];
}
...
async function readState(): Promise<StatePayload> {
  ...
  const opened = session.votingOpen
  ...
  return {
    ...
    openedCandidateIds: opened.map((candidate) => candidate.id),
  };
}
